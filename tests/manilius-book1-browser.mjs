...
try {
 ...
 assert.deepEqual(await page.locator('#manilius-book1 .source-item a').evaluateAll(xs=>xs.map(a=>a.getAttribute('href'))),book.map(t=>`/translations/${t.id}/`));assert.match(await page.locator('#manilius-breiter1907 > p').textContent(),/실제 920행/);assert.match(await page.locator('#manilius-breiter1907 > p').textContent(),/제4권 585행 이후/);
