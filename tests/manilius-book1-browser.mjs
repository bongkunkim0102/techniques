...
try {
 ...
 assert.deepEqual(await page.locator('#manilius-book1 .source-item a').evaluateAll(xs=>xs.map(a=>a.getAttribute('href'))),book.map(t=>`/translations/${t.id}/`));assert.match(await page.locator('#manilius-breiter1907 > p').textContent(),/실제 920행/);assert.match(await page.locator('#manilius-breiter1907 > p').textContent(),/다섯 권의 현전 인쇄 본문 전체/);
 ...
 for(const [i,t] of added.entries()){
  ...
  if(i<added.length-1)assert.equal(await page.locator('a[rel=next]').getAttribute('href'),`/translations/${added[i+1].id}/`);else assert.equal(await page.locator('a[rel=next]').getAttribute('href'),'/translations/manilius-ii-001-024/');
 }
