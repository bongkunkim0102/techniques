...
try{
 ...
 for(const [book,count] of [[1,28],[2,36],[3,22],[4,35],[5,33]])assert.equal(await page.locator(`#manilius-book${book} .source-item`).count(),count);
 ...
 for(const [i,t] of units.entries()){
  ...
  if(i<units.length-1)assert.equal(await page.locator('a[rel=next]').getAttribute('href'),`/translations/${units[i+1].id}/`);else assert.equal(await page.locator('a[rel=next]').getAttribute('href'),'/translations/manilius-v-251-270/');
 }
 ...
 await page.goto(origin+'/translations/manilius-v-234-250/');assert.match(await page.locator('.latin-verse').last().textContent(),/crater umoris amator/);assert.equal(await page.locator('a[rel=next]').getAttribute('href'),'/translations/manilius-v-251-270/');
