...
test('volumes never interleave, Book I is complete, and Book II begins at I',()=>{
  ...
  assert.deepEqual(volumes.map(v=>v.id),['ptolemy-book1','ptolemy-book2','ptolemy-book3','ptolemy-book4']);
  ...
  assert.deepEqual(volumes[3].entries.map(t=>t.series.order),Array.from({length:10},(_,i)=>i+1));
