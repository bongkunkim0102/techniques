...
test('three books form distinct continuous reading series with historical safeguards',()=>{
 const volumes=translationVolumes(ts.filter(t=>t.sourceId==='manilius-breiter1907'));assert.deepEqual(volumes.slice(0,3).map(v=>v.entries.length),[28,36,22]);for(const v of volumes)assert.deepEqual(v.entries.map(t=>t.series.order),v.entries.map((_,i)=>i+1));
